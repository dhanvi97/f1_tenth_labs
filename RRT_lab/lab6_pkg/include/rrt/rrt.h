...
private:
    ...
    void create_local_occupancy_grid(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan_msg);
    // Data collection
    void map_callback(const nav_msgs::msg::OccupancyGrid::ConstSharedPtr map_msg);
    void readCSV(const std::string& filename);
    void find_next_goal(int closest_waypoint_index);
    int closest_point_on_waypoints();
    tuple<float, float> find_nearest_free_space(geometry_msgs::msg::Pose transformed_pose);
    geometry_msgs::msg::Pose transform_to_vehicle_frame(geometry_msgs::msg::Pose goal_pose);
    void visualize_waypoints();

    
    // RRT methods
    std::vector<double> sample(int iteration);
    ...
    float get_robot_yaw();
    nav_msgs::msg::Path upsample_path(std::vector<RRT_Node> &path);
    // RRT* methods
